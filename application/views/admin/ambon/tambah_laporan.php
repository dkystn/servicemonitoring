...
<!-- <script>
 ...
 <script>
    // Menangkap elemen tanggal
    var tanggalInput = document.getElementById('tanggal');

    // Menangkap elemen regional
    var regionalSelect = document.getElementById('regional');

    // Mendengarkan perubahan pada input tanggal
    tanggalInput.addEventListener('input', function() {
        // Mengecek apakah tanggal telah diisi
        if (tanggalInput.value) {
            // Mengaktifkan pilihan regional
            regionalSelect.disabled = false;
        } else {
            // Menonaktifkan pilihan regional jika tanggal belum diisi
 ...
 <script>
    $(document).ready(function() {
        // Menambahkan event listener ketika regional dipilih
        $('select[name="regional"]').change(function() {
            var selected_regional = $(this).val(); // Mengambil nilai regional yang dipilih

            // Memuat opsi jenis perjalanan berdasarkan regional yang dipilih
            $.ajax({
                url: '<?php echo base_url("Admin/get_cabang_by_regional"); ?>',
                type: 'POST',
                data: { selected_regional: selected_regional },
                dataType: 'json',
                success: function(response) {
                    // Menghapus semua opsi yang ada setelah opsi pertama
                    $('select[name="cabang"] option:gt(0)').remove();

                    // Menambahkan opsi jenis perjalanan yang baru
                    $.each(response, function(index, value) {
                        $('select[name="cabang"]').append('<option value="'+ value.id_cabang + '">' + value.cabang +   value.id_cabang + '</option>');
                    });

                    // Menampilkan elemen dengan id="type_cabang"
                    // $('#type_cabang').show();
                }
            });
        });
 ...
 <script>
    $(document).ready(function() {
        // Menambahkan event listener ketika cabang dipilih
        $('select[name="cabang"]').change(function() {
            var selectedCabang = $(this).val(); // Mengambil nilai cabang yang dipilih

            // Memuat opsi jenis perjalanan berdasarkan cabang yang dipilih
            $.ajax({
                url: '<?php echo base_url("Admin/getType_journey"); ?>',
                type: 'POST',
                data: { cabang: selectedCabang },
                dataType: 'json',
                success: function(response) {
                    // Menghapus semua opsi yang ada sebelumnya
                    $('select[name="type_journey"] option:gt(0)').remove();

                    // Menambahkan opsi jenis perjalanan yang baru
                    $.each(response, function(index, value) {
                        $('select[name="type_journey"]').append('<option value="'+ value.journey + '|' + value.id_journey + '">' + value.journey +   value.id_journey + '</option>');
                    });

                    // Menampilkan elemen dengan id="type_journey"
                    // $('#type_journey').show();
                }
            });
        });
...
<script>
    $(document).ready(function() {
            // Menambahkan event listener ketika type_journey dipilih
            $('select[name="type_journey"]').change(function() {
                        var selectedtype_journey = $(this).val(); // Mengambil nilai type_journey yang dipilih
                        value_all = selectedtype_journey
                        value_all.split("|");
                        arrSplit = value_all.split("|");
                        arrSplit[0];
                        var value_0 = arrSplit[0];
                        var value_1 = arrSplit[1];
                        // Memuat opsi jenis perjalanan berdasarkan type_journey yang dipilih
                        console.log("cek journey" + value_1);
                        if (value_0 === "Pre Journey") {
                            $.ajax({
                                url: '<?php echo base_url("Admin/getJourneyOptions_type_journey"); ?>',
                                type: 'POST',
                                data: { type_journey: value_1 },
                                dataType: 'json',
                                success: function(response) {
                                    // console.log("cek response" + response);
                                    $('select[name="type_option"] option:gt(0)').remove();
                                    // Menambahkan opsi jenis perjalanan yang baru
                                    $.each(response, function(index, value) {
                                        $('select[name="type_option"]').append('<option value="' + value.id_type_option + '">' + value.type_option +   value.id_type_option + '</option>');
                                    });

                                    // // Menampilkan elemen dengan id="type_option"
                                    // $('#type_option').show();
                                }
                            });
                        } else if (value_0 === "Port Journey"){
                            ...
                            $.ajax({
                                url: '<?php echo base_url("Admin/getPelabuhanOptions_post"); ?>',
                                type: 'POST',
                                data: { cabang: cabang },
                                dataType: 'json',
                                success: function(response) {
                                    // Menambahkan opsi jenis perjalanan yang baru
                                    $('select[name="pelabuhan"] option:gt(0)').remove();
                                    $.each(response, function(index, value) {
                                        $('select[name="pelabuhan"]').append('<option value="' + value.id_pelabuhan + '">' + value.pelabuhan +   value.id_pelabuhan + '</option>');
                                    });

                                    // // Menampilkan elemen dengan id="pelabuhan"
                                    // $('#type_option').show();
                                }
                            });
                            ...
                            var cabang = $('select[name="cabang"]').val();
                            $.ajax({
                                url: '<?php echo base_url("Admin/getKapalOptions_post"); ?>',
                                type: 'POST',
                                data: { cabang: cabang },
                                dataType: 'json',
                                success: function(response) {
                                    // Menambahkan opsi jenis perjalanan yang baru
                                    $('select[name="kapal"] option:gt(0)').remove();
                                    $.each(response, function(index, value) {
                                        $('select[name="kapal"]').append('<option value="' + value.id_kapal + '">' + value.kapal +'</option>');
                                    });
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    // console.log("cek response" + response);
                                    // Menambahkan opsi jenis perjalanan yang baru
                                    $('select[name="type_option2"] option:gt(0)').remove();
                                    $.each(response, function(index, value) {
                                        $('select[name="type_option2"]').append('<option value="' + value.id_type_option + '">' + value.type_option +   value.id_type_option + '</option>');
                                    });
 ...
 <script>
    $(document).ready(function() {
        ...
        $('select[name="pelabuhan"]').change(function() {
            ...
            $.ajax({
                url: '<?php echo base_url("Admin/getJourneyOptions_pelabuhan"); ?>',
                type: 'POST',
                data: { pelabuhan: selectedPelabuhan },
                dataType: 'json',
                success: function(response) {
                    // Menghapus semua opsi yang ada sebelumnya
                    $('select[name="type_option3"] option:gt(0)').remove();
 ...
 <script>
    $(document).ready(function() {
        ...
        $('select[name="kapal"]').change(function() {
            ...
            $.ajax({
                url: '<?php echo base_url("Admin/getJourneyOptions_kapal"); ?>',
                type: 'POST',
                data: { kapal: selectedkapal },
                dataType: 'json',
                success: function(response) {
                    // Menghapus semua opsi yang ada sebelumnya
                    $('select[name="type_option4"] option:gt(0)').remove();
